...
| Temporality | STR | ↑ | 0.8333 | proxy_no_queue_model | 用停留時間落在推薦區間的程度近似，未納入排隊時間。 |
| Temporality | DTU | ↑ | 0.4659 | proxy_no_queue_model | 用每日景點時數 / 當日行程 span 近似。 |
| Temporality | OTU | ↑ | 0.1597 | proxy_no_queue_model | 用總景點時數 / 24h*天數近似。 |
| Economy | BE | ↑ | 27.2313 | proxy_estimated_costs | TravelEval 公式方向：experience value / actual cost * 1000；目前成本含住宿、景點票、餐費與市內交通估算，仍缺城際交通。 |
| Economy | ACD | - | 302.8000 | estimated_or_computed | 住宿成本。 |
| Economy | ATD | - | 660.0000 | estimated_or_computed | 景點票價優先來自 slot cost_estimate，缺少時用 TravelEval attraction price fallback。 |
| Economy | ETD | - | 0.0000 | missing_or_zero | 城際交通成本目前缺資料。 |
| Economy | RTD | - | 0.0000 | missing_or_zero | 市內交通成本優先來自 slot cost_estimate，缺少時用每段整團 8 CNY fallback。 |
| Economy | MED | - | 440.0000 | estimated_or_computed | 餐費優先來自 slot cost_estimate，缺少時用 TravelEval restaurant price 或每人 80 CNY fallback。 |
| Economy | OTD | - | 0.0000 | missing_or_zero | 其他成本。 |
| Utility | EDI | ↑ | 0.6466 | computed_partial | 用 TravelEval attraction type 做 Shannon entropy。 |
| Utility | ADS | ↑ | 0.5650 | computed_partial | 平均每日景點數 / 普通旅行 target density 3.54。 |
| Utility | AQE | ↑ | 0.9513 | computed_partial | 用景點 star/5 加權停留時間近似。 |
| Utility | Profit | ↑ | 6.3667 | computed_partial | 用 sandbox star 與偏好匹配近似。 |
