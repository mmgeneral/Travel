    ...
    *,
    ...
    for event_idx, event in enumerate(world.events):
        ...
        if revision:
            ...
            # satisfied event: no revision, no ask, no pairwise
            state.regret_trace.append(0.0)

        # ---- one posterior snapshot per event ----
        state.posterior_trace.append({
            "event_idx": event_idx,
            "mu": state.mu.tolist(),
            "Sigma_diag": np.diag(state.Sigma).tolist(),
            "proposal": int(x_sys),
            "true_best": int(y_true),
            "revision": bool(revision),
            "clarification_count_so_far": state.clarification_count,
            "pairwise_count_so_far": state.pairwise_count,
        })
